...
def api():
	...
	for command in commandsList:
		...
		# Call installableFonts() method, hand over root object to fill with data
			...
			success, message = installableFonts(root, subscriptionURL, APIKey, subscriptionID, secretKey, accessToken, \
				anonymousAppID, anonymousTypeWorldUserID, verifiedTypeWorldUserCredentials)
		...
		# Call installFonts() method, hand over root object to fill with data
			...
			# Call installFonts()
			success, message = installFonts(root, fonts, subscriptionURL, APIKey, subscriptionID, secretKey, accessToken, \
				anonymousAppID, anonymousTypeWorldUserID, verifiedTypeWorldUserCredentials, userName, userEmail)
		...
		# Call uninstallFonts() method, hand over root object to fill with data
			...
			# Call uninstallFonts()
			success, message = uninstallFonts(root, fonts, subscriptionURL, APIKey, subscriptionID, secretKey, accessToken, \
				anonymousAppID, anonymousTypeWorldUserID, verifiedTypeWorldUserCredentials)
...
def endpoint(root):
	...
	endpoint.supportedCommands = ['endpoint', 'installableFonts', 'installFonts', 'uninstallFonts']
	# etc ...
...
def installableFonts(root, subscriptionURL, APIKey, subscriptionID, secretKey, accessToken, anonymousAppID, \
	anonymousTypeWorldUserID, verifiedTypeWorldUserCredentials):
	'''
	...
	if subscriptionID:
		...
		if securityCheckPassed == False:
			...
			# Has not been verified yet, so we need to verify them now
				...
				verifiedTypeWorldUserCredentials = verifyUserCredentials(APIKey, anonymousAppID, anonymousTypeWorldUserID, subscriptionURL)
...
def installFonts(root, fonts, subscriptionURL, APIKey, subscriptionID, secretKey, accessToken, anonymousAppID, anonymousTypeWorldUserID, \
	verifiedTypeWorldUserCredentials, userName, userEmail):
	'''
	...
	success, message = createInstallFontsObjectTree(installFonts, fonts, subscriptionID, anonymousAppID, userName, userEmail, __ownDataSource__)
...
def uninstallFonts(root, fonts, subscriptionURL, APIKey, subscriptionID, secretKey, accessToken, anonymousAppID, anonymousTypeWorldUserID, \
	...
	success, message = createUninstallFontsObjectTree(installFonts, fonts, subscriptionID, anonymousAppID, __ownDataSource__)
...
def createInstallFontsObjectTree(installFonts, fonts, subscriptionID, anonymousAppID, userName, userEmail, __ownDataSource__):
	'''
	...
	for fontID, fontVersion in fontsList:
		...
		if __fontDataSource__.__protected__:
			...
			if __fontDataSource__.__isTrialFont__:
				...
				if seats == None:
					__ownDataSource__.__recordFontInstallation__(fontID, fontVersion, subscriptionID, anonymousAppID, userName, userEmail)

				# Font has been previously installed (so a record exists), but is marked as 'uninstalled', so we update that
				else:
					__ownDataSource__.__updateFontInstallation__(fontID, subscriptionID, anonymousAppID, trialInstalledStatus = True)
...
def createUninstallFontsObjectTree(uninstallFonts, fonts, subscriptionID, anonymousAppID, userName, userEmail, __ownDataSource__):
	...
	for fontID in fontsList:
		...
		if __fontDataSource__.__protected__:
			...
			if __fontDataSource__.__isTrialFont__:
				__ownDataSource__.__updateFontInstallation__(fontID, subscriptionID, anonymousAppID, trialInstalledStatus = False)
...
def verifyUserCredentials(APIKey, anonymousAppID, anonymousTypeWorldUserID, subscriptionURL = None):
	'''
